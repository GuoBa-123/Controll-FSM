 ...
 */
 ...
 private:
     ...
     typedef struct
     {
         ...
         bool *reason;     // 指向外部 bool 變量，為 true 時觸發轉移
     } stateTrans_t;
 ...
 public:
      ...
      * @param   from  來源狀態名稱
      * @param   to    目標狀態名稱
      * @param   reason  外部 bool 變量的引用，當其變為 true 時觸發轉移
      *                （觸發後自動重置為 false）
      * @return  true  註冊成功
      * @return  false 來源或目標狀態不存在，或轉移表已滿
      */
     bool stateSetTrans(const char *from, const char *to, bool &reason)
     {
         ...
         stateTransForm[transCount] = {fromId, toId, &reason};
         transCount++;
         return true;
     }
     ...
     void stateUpdate()
     {
         ...
         {
             if (stateTransForm[i].fromID == currentID && *stateTransForm[i].reason)
             {
                 *stateTransForm[i].reason = false;       // 觸發後自動復位
